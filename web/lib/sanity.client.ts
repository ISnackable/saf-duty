...
import {
  ...
  getAllUsersQuery,
  getCalendarQuery, // getUserPushSubscriptionQuery,
...
export async function getUserUpcomingDuties(
  id: string
): Promise<TDateISODate[]> {
  ...
  const result = await client.fetch(getUserUpcomingDutiesQuery, {
    id,
    firstDay,
    lastDay,
  });
...
export async function getUserBlockouts(
  id: string
): Promise<SanityUserBlockouts> {
...
export async function getAllUserCalendar(id: string): Promise<Calendar[]> {
  const calendar: Calendar[] = await clientWithToken.fetch(
    getAllUserCalendarQuery,
    { id }
  );
...
export async function getCalendarById(id: string): Promise<Calendar> {
  const calendar: Calendar = await clientWithToken.fetch(getCalendarQuery, {
    id,
  });
...
export async function getUserSwapRequest(
  id: string
): Promise<SanitySwapRequest[]> {
