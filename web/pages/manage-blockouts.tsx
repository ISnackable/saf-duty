...
export default function ManageBlockoutPage() {
  ...
  const handleSelect = (date: Date) => {
    ...
    const { weekends, fridays } = calculateWeekendsAndFridays(
      currentMonth,
      lastDayOfMonth
    );
    ...
    const numSelectedFridays = countSelectedDates(
      fridays,
      currentMonthSelected
    );
    ...
    if (isSelected) {
      ...
      if (
        (dayjs(date).day() === 5 && !canSelectFriday) ||
        (isWeekend(date) && !canSelectWeekend)
      ) {
        return;
      }
  ...
  function calculateWeekendsAndFridays(
    currentMonth: dayjs.Dayjs,
    lastDayOfMonth: dayjs.Dayjs
  ) {
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/sanity/user/${session?.user?.id}/blockouts`,
        {
          method: 'PUT',
          headers: {
      ...
      if (resData?.status === 'error') {
        showNotification({
          ...
          message:
            resData?.message ||
            'Cannot update block out dates, something went wrong',
          color: 'red',
          icon: <IconX />,
        });
      } else {
  ...
  return (
    <Container my='xl' size='xl'>
      ...
      <List mt='lg'>
        <List.Item>
          Only a maximum of {maximumBlockouts} blockouts date per month (subject
          to change)
        </List.Item>
        <List.Item>
          Inform the person-in-charge if you need more blockouts
        </List.Item>
        <List.Item>
          You are not allowed to blockout every <u>weekend & friday</u> of the
          month
        </List.Item>
      </List>
      <Divider mt='sm' />
