...
export default function IndexPage() {
  ...
  const [drawerDateValue, setDrawerDateValue] = useState<
    Date | null | undefined
  >(null);
  const [drawerPersonnelValue, setDrawerPersonnelValue] = useState<
    Pick<SanityUser, 'id' | 'name'> | null | undefined
  >(null);
  const [drawerSelectValue, setDrawerSelectValue] = useState<string | null>(
    null
  );
  ...
  const dutyDates = calendar?.find((cal) =>
    isSameMonth(new Date(cal.date), month)
  );
      ...
      const res = await fetch(
        `/api/sanity/user/${session.user.id}/swap-request`,
        {
          ...
          body: JSON.stringify({
            ...
            requesterDate: new Date(drawerSelectValue).toLocaleDateString(
              'sv-SE'
            ),
            reason,
          }),
              ...
              onClick: () => {
                ...
                if (dutyDates) {
                  ...
                  if (date.valueOf() <= new Date().valueOf()) {
                    ...
                    userId &&
                    !(userId === dutyDates?.roster?.[day - 1]?.personnel?.id)
                  ) {
                    if (
