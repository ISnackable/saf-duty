...
async function handler(req: NextApiRequestWithUser, res: NextApiResponse) {
  ...
  if (method === 'GET') {
    try {
      ...
      return res
        .status(200)
        .json({ status: 'success', data: calendar, message: 'ok' });
    } catch (error) {
      console.error(error);
      return res
        .status(500)
    ...
    const {
      dutyDates,
      dutyPersonnel,
    }: { dutyDates: DutyDate[]; dutyPersonnel: Personnel[] } = req.body;

    const roster = dutyDates.map((dutyDate) => {
      const dutyPerson = dutyPersonnel.find(
        (person) => person.name === dutyDate.personnel
      );
      const dutyPersonStandIn = dutyPersonnel.find(
        (person) => person.name === dutyDate.standby
      );
    ...
    try {
      ...
      return res
        .status(201)
        .json({ status: 'success', message: 'Calendar updated' });
    } catch (error) {
