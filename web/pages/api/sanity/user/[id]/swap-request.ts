...
async function handler(req: NextApiRequestWithUser, res: NextApiResponse) {
  ...
  if (method === 'GET') {
    try {
      ...
      return res
        .status(200)
        .json({ status: 'success', data: swapRecords, message: 'ok' });
    } catch (error) {
      console.error(error);
    ...
    try {
      ...
      console.log(
        `new swap request was created, document ID is ${sanityRes._id}`
      );
    ...
    if (action === 'approve') {
      try {
        ...
        if (!calendarDoc) {
          return res
            .status(404)
            .json({ status: 'error', message: 'Calendar not found' });
        }
        ...
        if (requesterRosterIndex === -1 || receiverRosterIndex === -1) {
          await clientWithToken.delete(
            `${requesterDate}-${requester}-${receiverDate}-${receiver}`
          );
        ...
        if (
          previousRequesterRosterIndex >= 0 &&
          previousReceiverRosterIndex >= 0
        ) {
        ...
        if (
          nextRequesterRosterIndex < roster.length &&
          nextReceiverRosterIndex < roster.length
        ) {
        ...
        if (
          ...
          (previousRequesterRoster &&
            previousRequesterRoster.personnel.id ===
              requesterRoster.standby.id) ||
          (previousRequesterRoster &&
            previousRequesterRoster.standby.id ===
              requesterRoster.standby.id) ||
          (nextRequesterRoster &&
            nextRequesterRoster.personnel.id === requesterRoster.standby.id) ||
          (nextRequesterRoster &&
            nextRequesterRoster.standby.id === requesterRoster.standby.id)
        ) {
        ...
        if (
          ...
          (previousReceiverRoster &&
            previousReceiverRoster.personnel.id ===
              receiverRoster.standby.id) ||
          (previousReceiverRoster &&
            previousReceiverRoster.standby.id === receiverRoster.standby.id) ||
          (nextReceiverRoster &&
            nextReceiverRoster.personnel.id === receiverRoster.standby.id) ||
          (nextReceiverRoster &&
            nextReceiverRoster.standby.id === receiverRoster.standby.id)
        ) {
      ...
      try {
        ...
        return res
          ...
          .json({
            status: 'success',
            message: 'Successfully declined the swap request',
          });
      } catch (error) {
        return res
          .status(500)
    ...
    try {
      ...
      return res
        ...
        .json({
          status: 'success',
          message: 'Successfully deleted the swap request',
        });
    } catch (error) {
      return res
        .status(500)
