...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    if (!isValidSignature) {
      ...
      return res
        .status(401)
        .send({ status: 'error', message: `Unauthorized, ${message}` });
    } else if (!body)
      return res
        .status(400)
        .json({ status: 'error', message: 'Bad request, empty body' });
    ...
    const rosterLocaleDateString = new Date(
      body.date as string
    ...
    let dutyPersonnelsWithSubscriptions: DutyPersonnel[] =
      body.dutyPersonnels as DutyPersonnel[];
    ...
    dutyPersonnelsWithSubscriptions = dutyPersonnelsWithSubscriptions.filter(
      ({ id, subscription }, index) =>
        !ids.includes(id, index + 1) && subscription
    );
    ...
    if (!dutyPersonnelsWithSubscriptions.length)
      return res
        .status(404)
        .json({ status: 'error', message: 'Subscriptions not found' });
