...
export default function Example() {
  ...
  let selectedDayMeetings = duties.filter((meeting) =>
              ...
              {days.map((day, dayIdx: number) => (
                ...
                >
                  ...
                  <div className="w-1 h-1 mx-auto mt-1">
                    {duties.some((duty) =>
                      isSameDay(parseISO(duty.startDatetime), day)
                    ) && (
          ...
          <section className="mt-12 md:mt-0 md:pl-14">
            ...
            <ol className="mt-4 space-y-1 text-sm leading-6 text-gray-500">
              {selectedDayMeetings.length > 0 ? (
                selectedDayMeetings.map((duty) => (
                  <Duties duty={duty} key={duty.id} />
                ))
              ) : (
...
function Duties({
  ...
  let startDateTime = parseISO(duty.startDatetime);
  let endDateTime = parseISO(duty.endDatetime);
  ...
  return (
    <li className="flex items-center px-4 py-2 space-x-4 group rounded-xl focus-within:bg-gray-100 hover:bg-gray-100">
      ...
      <div className="flex-auto">
        <p className="text-gray-900">{duty.name}</p>
        <p className="mt-0.5">
          <time dateTime={duty.startDatetime}>
            {format(startDateTime, "h:mm a")}
          </time>{" "}
          -{" "}
          <time dateTime={duty.endDatetime}>
