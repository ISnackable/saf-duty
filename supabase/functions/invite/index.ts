...
if (!(SUPABASE_URL && SUPABASE_SERVICE_ROLE_KEY && SB_JWT_SECRET)) {
  ...
  Promise.resolve()
    .then(async function () {
      ...
      if (group_users.length == 0) {
        ...
        return response
          .setStatus(500)
          .send('No roles assigned for invite code');
      }

      const res2 = await supabase.from('group_users').upsert(group_users, {
