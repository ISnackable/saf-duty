...
declare
  new_unit TEXT := new.raw_user_meta_data->>'unit';
  unit_id bigint := (SELECT id from public.units WHERE unit_code=new_unit);
begin
insert into
...
CREATE OR REPLACE TRIGGER "on_after_notifications_created" AFTER INSERT ON "public"."notifications" FOR EACH ROW EXECUTE FUNCTION "supabase_functions"."http_request"('https://tscbuvxcsuxlqwcewtgu.supabase.co/functions/v1/push', 'POST', '{"Content-type":"application/json"}', '{}', '1000');

CREATE OR REPLACE TRIGGER "on_before_swap_request_created" BEFORE INSERT ON "public"."swap_requests" FOR EACH ROW EXECUTE FUNCTION "public"."validate_swap_request"();

CREATE OR REPLACE TRIGGER "profile_cls" BEFORE INSERT OR UPDATE ON "public"."profiles" FOR EACH ROW EXECUTE FUNCTION "public"."allow_updating_only"('name', 'avatar_url', 'enlistment_date', 'ord_date', 'blockout_dates', 'updated_at');
...
CREATE POLICY "Enable read access for users based on their unit_id" ON "public"."roster" FOR SELECT USING (("unit_id" = (( SELECT (("auth"."jwt"() -> 'app_metadata'::"text") -> 'unit_id'::"text")))::bigint));
...
CREATE POLICY "Public profiles are viewable by everyone in the same unit." ON "public"."profiles" FOR SELECT USING ((("public"."get_my_claim"('unit_id'::"text"))::bigint = "unit_id"));
