...
ALTER TABLE "public"."rosters" ALTER COLUMN "id" ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME "public"."rosters_id_seq"
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE TRIGGER on_after_rosters_created AFTER INSERT ON public.rosters REFERENCING NEW TABLE AS new_table FOR EACH STATEMENT EXECUTE FUNCTION public.handle_rosters_notification();
...
CREATE POLICY "Enable insert for 'admin' role only" ON "public"."rosters" FOR INSERT WITH CHECK ((public.has_group_role(group_id, 'admin'::text)));
...
CREATE POLICY "Enable read access for users based on their group_id" ON "public"."rosters" FOR SELECT USING ((public.is_group_member(group_id)));
...
CREATE POLICY "Enable update for 'admin' role only" ON "public"."rosters" FOR UPDATE USING ((public.has_group_role(group_id, 'admin'::text))) WITH CHECK ((public.has_group_role(group_id, 'admin'::text)));
