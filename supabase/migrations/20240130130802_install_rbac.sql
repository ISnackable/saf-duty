...
BEGIN
    IF session_user = 'authenticator' THEN
        ...
        select coalesce(get_req_groups(), auth.jwt()->'app_metadata'->'groups')->'id' = to_jsonb(group_id)
            and coalesce(get_req_groups(), auth.jwt()->'app_metadata'->'groups')->'role' = to_jsonb(group_role)
        into retval;
        return retval;
    ELSE -- not a user session, probably being called from a trigger or something
        return true;
