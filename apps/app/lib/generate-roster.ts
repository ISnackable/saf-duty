...
function createDutyDate(
  ...
  return date.flatMap((date) => {
    if (isDateInArray(omitDates, date)) {
      return [];
    }
...
function allocateWeekdayShift(
  ...
  for (const person of shuffledDutyPersonnel.slice(
    0,
...
function allocateWeekendShift(
  ...
  const shuffledDutyPersonnel = shuffleArray(personnel);
