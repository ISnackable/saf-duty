...
import { getProfilesByUserId } from '@/lib/data';
...
export const GET = withAuth(
  async ({ user }) => {
    try {
      const data = await getProfilesByUserId(user.organizationId, user.id);
...
export const PATCH = withAuth(async ({ request, params, user }) => {
  try {
    ...
    if (params.userId !== user.id && !['admin', 'owner'].includes(userRole)) {
      return NextResponse.json(
        {
          status: 'error',
  ...
  async ({ params, user }) => {
    try {
      ...
      return NextResponse.json(
        {
          status: 'success',
          message: `Successfully deleted profile with id: ${params.userId}`,
        },
        { status: 200 }
      );
    } catch (_error) {
