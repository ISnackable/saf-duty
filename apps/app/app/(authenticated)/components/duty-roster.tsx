...
import type { NagerDatePublicHoliday } from '@/hooks/use-public-holidays';
...
import {
  BentoGrid,
  BentoGridItem,
} from '@repo/design-system/components/bento-grid';
import { Icons } from '@repo/design-system/components/icons';
import { Indicator } from '@repo/design-system/components/indicator';
...
const dutyDateSchema = z.object({
  ...
  dutyPersonnel: personnelSchema,
  reserveDutyPersonnel: personnelSchema,
  allocated: z.boolean(),
});
...
function DayWithTime(
  ...
  return (
    <div className="relative h-full w-full">
      {isPublicHoliday && (
        <span className="absolute top-1.5 left-1 text-red-600 text-xs sm:left-2">
          PH
        </span>
      )}
      <time
        dateTime={date}
        className="sm:top:3 absolute top-1 right-2 text-sm sm:right-4 sm:text-lg"
      >
        <DayContent {...props} />
      </time>

      {/* biome-ignore lint/suspicious/noPrototypeBuiltins: <explanation> */}
      {roster?.hasOwnProperty(date) && (
        <Indicator
          disabled={roster[date]?.dutyPersonnel?.id !== session?.user?.id}
          placement="top-right"
        ...
        >
          ...
          >
            <span>
              {roster[date]?.dutyPersonnel?.name ?? ''}{' '}
              {roster[date]?.isExtra ? 'EX ' : ''}
            </span>
            <span>{`(${roster[date]?.reserveDutyPersonnel?.name ?? ''})`}</span>
          </div>
        </Indicator>
      )}
    </div>
...
export function DutyRoster() {
  ...
  const [dutyRoster, setDutyRoster] = useState<Record<string, Rosters>>({});
          ...
          onClick: () => {
            window.location.href = `whatsapp://send?text=${encodeURIComponent(
              `Hello, I would like to swap duty with you on ${format(
                values.receiverRoster.dutyDate,
                'PPPP'
    ...
    <Credenza open={open} onOpenChange={setOpen}>
      <Calendar
        ...
        onDayClick={(day) => {
          ...
          if (
            !roster ||
            !roster?.dutyPersonnel ||
            !roster?.reserveDutyPersonnel ||
            !roster?.id
          ) {
            return;
          }
          ...
          if (roster?.dutyPersonnel?.id === session.user.id) {
            toast.warning("You can't swap duty with yourself!");
            return;
          }

          if (
            dutyRoster[prevDate]?.dutyPersonnel?.id === session.user.id ||
            dutyRoster[nextDate]?.dutyPersonnel?.id === session.user.id
          ) {
            toast.warning(
            ...
            !Object.values(dutyRoster).some(
              (v) => v?.dutyPersonnel?.id === session.user.id
            )
          ) {
          ...
          credenzaForm.setValue('receiverRoster', {
            ...roster,
            id: roster.id,
            dutyPersonnel: roster.dutyPersonnel,
            reserveDutyPersonnel: roster.reserveDutyPersonnel,
          });
          credenzaForm.setValue('receiver', roster.dutyPersonnel);
        ...
        classNames={{
          ...
          day: cn(
            ...
            'h-full w-full rounded-none p-0 font-normal text-lg aria-selected:bg-[#fa5858] aria-selected:opacity-90'
          ),
                ...
                <div className="space-y-2">
                  <BentoGrid className="md:auto-rows mx-auto max-w-4xl grid-cols-2 md:grid-cols-2">
                    <BentoGridItem
                      ...
                      title="Date"
                      description={format(
                        credenzaForm.getValues('receiverRoster').dutyDate,
                        'PPPP'
                      )}
                        ...
                        >
                          ...
                          <SelectContent side="top">
                            {Object.values(dutyRoster)
                              .filter(
                                (roster) =>
                                  roster?.dutyPersonnel?.id ===
                                    session?.user?.id &&
                                  new Date(roster?.dutyDate).getTime() >
                                    TODAY.getTime() &&
                                  isSameMonth(
                                    monthDate,
                                    new Date(roster?.dutyDate)
                                  )
                              )
                              .map((roster) => (
                                <SelectItem
                                  key={roster.id}
                                  value={roster.dutyDate}
                                >
                                  {format(roster.dutyDate, 'ccc, PP')}
                                </SelectItem>
                              ))}
                          </SelectContent>
                        </Select>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Textarea
                            className="!text-sm resize-none placeholder:text-sm"
