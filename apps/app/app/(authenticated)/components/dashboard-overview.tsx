...
export function DashboardOverview() {
  ...
  const profile = React.useMemo(
    () => profiles?.find((profile) => profile.userId === session?.user?.id),
    [profiles, session]
  );
  ...
  const onClickAddToCalendar = async () => {
    ...
    const calendarConfig: ATCBActionEventConfig = {
      ...
      dates: upcomingDuties.map((roster) => ({
        ...
        name: 'SAF Duty',
        description: `You have a duty on ${roster.dutyDate} at 8am. Please be on time! This was automatically generated by the SAF Duty Roster at [url]${host}[/url].`,
        startDate: roster.dutyDate,
      })),
      customLabels: {
  ...
  React.useEffect(() => {
    ...
    const index = upcomingDuties.findIndex((roster) => {
      return isFuture(roster.dutyDate);
    });
              ...
              {profiles && profiles.length > 0
                ? profiles.map((profile) => (
                    ...
                    >
                      ...
                      <p className="pt-2 text-center font-medium text-sm leading-none">
                        {profile.name}
                      </p>
                    </CarouselItem>
                  ))
