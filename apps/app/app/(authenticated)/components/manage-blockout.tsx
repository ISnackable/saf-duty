...
export function ManageBlockout() {
  ...
  const maximumBlockouts = profile?.maxBlockouts || MAXIMUM_BLOCKOUTS;
  ...
  const updateBlockoutDates = async () => {
    ...
    const resPromise = fetcher(
      `/api/organizations/${session.session.activeOrganizationId}/profiles/${profile?.userId}/blockout-dates`,
      {
        method: 'POST',
        body: JSON.stringify({
  ...
  const handleDayClick: DayClickEventHandler = useCallback(
    (day, modifiers) => {
      ...
      if (modifiers.selected) {
        ...
        if (isFriday(day) && !canSelectFriday) {
          toast.warning(
